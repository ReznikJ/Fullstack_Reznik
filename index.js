...
app.get('/api/auta', (req, res) => res.json(nactiData()));

app.post('/api/auta', (req, res) => {
    const auta = nactiData();
    const noveAuto = { id: Date.now(), prodano: false, ...req.body };
    auta.push(noveAuto);
    ulozData(auta);
    res.json(noveAuto);
});
...
app.put('/api/auta/:id', (req, res) => {
    ...
    const index = auta.findIndex(a => a.id === id);
    if (index !== -1) {
        auta[index] = { ...auta[index], ...req.body };
        ulozData(auta);
        res.json(auta[index]);
    } else {
